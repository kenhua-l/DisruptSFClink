export const subFunction = () => {
	const url = "https://api.crunchbase.com/v/3/organizations?user_key=eef6a0d4bb64dca85b2026bb3550f768";
	console.warn('something');
	fetch(url).then((resp) => {
		console.warn('asd');
		resp.json().then(resp => console.warn(resp));
	}, respF => {
		console.warn('failed');
	});
	console.warn('ending');
...
export const superFunction = () => {
	...
	var permalink;
	function ff() {
		fetch(apiOrganizationDataURL, function(data, status) {
	...
	function f() {
		fetch(apiFullOrganizationDataURL, function(data, status) {
